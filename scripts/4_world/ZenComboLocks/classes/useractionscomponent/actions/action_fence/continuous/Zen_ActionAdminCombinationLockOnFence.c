...
class Zen_ActionAdminCombinationLockOnFence : ActionContinuousBase
{
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		if (!player.IsZenAdmin() || !GetZenComboLocksConfig().ClientSyncConfig.AllowAdminOpen)
			return false;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		// If player does not have admin authority, stop here
		if (!action_data.m_Player || !action_data.m_Player.IsZenAdmin() || !GetZenComboLocksConfig().ClientSyncConfig.AllowAdminOpen)
			return;
