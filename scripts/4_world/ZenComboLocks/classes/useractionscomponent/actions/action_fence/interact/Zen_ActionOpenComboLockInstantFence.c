class Zen_ActionOpenComboLockInstantFence : ActionInteractBase
{
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		return lock.IsLocked() && (GetZenComboLocksConfig().ClientSyncConfig.InstantOpen && lock.IsPermittedToOpen(player) && !lock.IsManagingLockClient()) || player.IsZenAdmin();
	}
	...
	override void OnStartServer(ActionData action_data)
	{
		...
		if (GetZenComboLocksConfig().ClientSyncConfig.InstantOpen || (GetZenComboLocksConfig().ClientSyncConfig.AllowAdminOpen && action_data.m_Player.IsZenAdmin()))
		{
